...
```
                                ...
                                ┌─────────────────────────────┐
                                │  DNS (Route53 / DNSPod)     │
                                │  api.example.com → AWS-ELB  │
                                └──────────────┬──────────────┘
                                               │
                                               ▼
╔═══════════════════════════════════════════╗       ╔═══════════════════════════════════════════╗
║            AWS Cloud (us-east-1)          ║       ║          腾讯云 (ap-guangzhou)            ║
║                                           ║       ║                                           ║
║  ┌─────────────────────────────────────┐ ║       ║                                           ║
║  │       ELB (Load Balancer)           │ ║       ║                                           ║
║  └──────────────┬──────────────────────┘ ║       ║                                           ║
...
║  ┌─────────────────────────────────────┐ ║       ║                                           ║
║  │  App Servers (EC2)                  │ ║       ║                                           ║
║  │  [App-1] [App-2] [App-3]            │ ║       ║              （暂未使用）                  ║
║  │                                     │ ║       ║                                           ║
║  │  连接串:                             │ ║       ║                                           ║
║  │  mongodb://aws-1,aws-2,aws-3/       │ ║       ║                                           ║
║  └──────────────┬──────────────────────┘ ║       ║                                           ║
║                 │ 同 VPC  < 1ms           ║       ║                                           ║
║                 ▼                         ║       ║                                           ║
║  ┌─────────────────────────────────────┐ ║       ║                                           ║
...
流量: 用户 → AWS ELB → AWS App → AWS DB (全链路在 AWS 内，同 VPC 低延迟)
```

### 阶段二：迁移中（腾讯云节点作为 SECONDARY 加入，数据跨云同步）

> 应用还在 AWS 跑，数据库新成员在腾讯云，**跨云同步通过专线**
...
║                 ▼                         ║       ║                                           ║
║  ┌─────────────────────────────────────┐ ║       ║  ┌─────────────────────────────────────┐  ║
║  │  MongoDB 副本集 rs0 (AWS 侧)        │ ║       ║  │  MongoDB 副本集 rs0 (腾讯云侧)      │  ║
║  │                                     │ ║       ║  │                                     │  ║
║  │    ┌─────────┐ ◄── 写 / 读          │ ║       ║  │                                     │  ║
║  │    │ PRIMARY │                      │ ║       ║  │                                     │  ║
║  │    │ aws-1   │  priority=10        ═╬═╬═══════╬═╬═►  ┌──────────────┐                 │  ║
║  │    └────┬────┘                      │ ║ 跨云  ║  │   │  SECONDARY   │ ◄── 新加入成员   │  ║
║  │         │ oplog (本地)              │ ║专线   ║  │   │  tc-1        │                 │  ║
║  │    ┌────┴────┐  ┌─────────┐         │ ║ <50ms ║  │   │  priority=0  │  ← Initial Sync │  ║
║  │    │SECONDARY│  │SECONDARY│         │ ║1Gbps  ║  │   │  votes=0     │    数据全量同步  │  ║
║  │    │ aws-2   │  │ aws-3   │         │ ║       ║  │   └──────────────┘                 │  ║
║  │    └─────────┘  └─────────┘         │ ║       ║  │         ▲                           │  ║
║  │                                     │ ║       ║  │         │ 持续 oplog 增量同步        │  ║
║  └─────────────────────────────────────┘ ║       ║  └─────────┼───────────────────────────┘  ║
...
流量: 用户 → AWS ELB → AWS App → AWS DB (主节点)
数据: AWS DB ═══跨云专线═══► 腾讯云 DB (后台异步同步, 不影响业务)
```

### 阶段三：切主完成（腾讯云成为 PRIMARY，但应用仍在 AWS——关键跨云写入阶段！）

> ⚠️ **这是最有意思的阶段**：DNS 和 App 还没切，但数据库主节点已在腾讯云。  
> AWS 的应用写入要**跨云专线**到腾讯云的 MongoDB，读取也是跨云！
...
║                 ▼                         ║       ║                                           ║
║  ┌─────────────────────────────────────┐ ║       ║  ┌─────────────────────────────────────┐  ║
║  │  MongoDB 副本集 rs0 (AWS 侧降级)    │ ║       ║  │  MongoDB 副本集 rs0 (腾讯云侧成主)  │  ║
║  │                                     │ ║       ║  │                                     │  ║
║  │    ┌─────────┐ 降级                 │ ║       ║  │   ┌─────────┐                       │  ║
║  │    │SECONDARY│ ◄════════════════════╬═╬═══════╬═╬═══┤ PRIMARY │ ◄── 写 / 读 (跨云!)    │  ║
║  │    │ aws-1   │ priority=1           │ ║       ║  │   │ tc-1    │    priority=10        │  ║
║  │    └─────────┘       oplog 反向同步 │ ║       ║  │   └────┬────┘    votes=1            │  ║
║  │    ┌─────────┐  ┌─────────┐         │ ║       ║  │        │ oplog                      │  ║
║  │    │SECONDARY│  │SECONDARY│         │ ║ 跨云  ║  │        │ 反向同步到 AWS            │  ║
║  │    │ aws-2   │  │ aws-3   │         │ ║ 专线  ║  │        ▼                            │  ║
║  │    └─────────┘  └─────────┘         │ ║       ║  │   (作为 Secondary 存在, 提供灾备)  │  ║
║  │                                     │ ║       ║  │                                     │  ║
║  └─────────────────────────────────────┘ ║       ║  └─────────────────────────────────────┘  ║
╚═══════════════════════════════════════════╝       ╚═══════════════════════════════════════════╝
  ...
  ⚠️ 本阶段特征:
  ┌──────────────────────────────────────────────────────────────────────────┐
  │ 1. 用户 → 仍访问 AWS App (DNS 未切)                                        │
  │ 2. AWS App → 写操作 driver 自动发现 PRIMARY 在腾讯云, 每次写入要跨云!        │
  │ 3. 读操作如配置 readPreference=primary, 也要跨云                           │
  │ 4. 跨云延迟 (~20-50ms) 叠加在每个请求上, 业务有感知 (RT 上升)               │
  │ 5. 所以这个阶段要尽快推进应用迁移, 不能停留太久                              │
  └──────────────────────────────────────────────────────────────────────────┘

流量: 用户 → AWS ELB → AWS App ═══跨云专线═══► 腾讯云 DB (主节点)
优化: readPreference=secondaryPreferred 可让读请求走 AWS 本地 Secondary
```
...
║   可完全下线或保留作归档                    ║       ║  │                                     │  ║
║                                           ║       ║  │  连接串 (最终版, 只含腾讯云):         │  ║
║                                           ║       ║  │  mongodb://tc-1,tc-2,tc-3/          │  ║
║                                           ║       ║  └──────────────┬──────────────────────┘  ║
║                                           ║       ║                 │ 同 VPC  < 1ms           ║
║                                           ║       ║                 ▼                         ║
║                                           ║       ║  ┌─────────────────────────────────────┐  ║
║                                           ║       ║  │  MongoDB 副本集 rs0 (纯腾讯云)      │  ║
║                                           ║       ║  │                                     │  ║
║                                           ║       ║  │    ┌─────────┐ ◄── 写 / 读          │  ║
║                                           ║       ║  │    │ PRIMARY │                      │  ║
║                                           ║       ║  │    │ tc-1    │                      │  ║
...
流量: 用户 → 腾讯云 CLB → 腾讯云 App → 腾讯云 DB (全链路在腾讯云，同 VPC 低延迟)
```

### 完整迁移时间线（含各层切换动作）

```
时间点      DNS         App 层            DB 层             流量走向
═══════════════════════════════════════════════════════════════════════════
D-30        →AWS-ELB    部署在 AWS         仅 AWS 3节点      用户→AWS→AWS-DB
  │
D-7         →AWS-ELB    部署在 AWS         腾讯云节点加入     用户→AWS→AWS-DB
  │                                       (priority=0)      (同步在后台进行)
  │
D-3         →AWS-ELB    更新连接串         Initial Sync 完成  用户→AWS→AWS-DB
  │                    包含 tc-1
  │
D-1         →AWS-ELB    同上              提升 tc-1         用户→AWS→AWS-DB
  │                                       priority=5
  │
D-0(低峰)   →AWS-ELB    同上              rs.stepDown()     短暂 1-10s 抖动
  │ 切主                                   tc-1 成为 PRIMARY  后自动恢复
  │                                                         用户→AWS→腾讯云-DB
  │                                                             (跨云访问)
  │
D+1         →AWS-ELB    开始部署到腾讯云    主节点在腾讯云      用户→AWS→腾讯云-DB
  │ 应用迁移
  │
D+3(低峰)   →腾讯云-CLB  流量切到腾讯云 App  主节点在腾讯云      用户→腾讯云→腾讯云-DB
  │ DNS 切换                              (同 VPC 低延迟)
  │
D+7+        →腾讯云-CLB  腾讯云为主         rs.remove() AWS    用户→腾讯云→腾讯云-DB
  │ 清理                AWS 应用下线        AWS 节点下线
  │
